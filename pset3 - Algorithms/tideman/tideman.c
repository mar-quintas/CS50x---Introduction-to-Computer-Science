...
int main(int argc, string argv[])
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        for (int j = 0; j < candidate_count; j++)
        {
            ranks[j] = 0; //aca decia -1 en vez de 0
        }
...
void sort_pairs(void)
{
    // here we will get the number of voters for a certain winner pair

    for (int i = 0; i < pair_count - 1; i++)
    {
        for (int j = 0; j < pair_count - i - 1; j++)
        {
            // Me falto hacer la resta entre ij ji para ver el strength of victory
...
// recursive function to use inside lock_pairs
bool valid(int winner, int loser)
{
    if (winner == loser)
    {
        return false;
...
void lock_pairs(void)
{
    ...
    {
        locked[pairs[i].winner][pairs[i].loser] = valid(pairs[i].winner, pairs[i].loser);
    }
